...
# 분석 대상 다크웹 타겟 URL
TARGET_URL = 'http://ockbit3753ekiocyo5epmpy6klmejchjtzddoekjlnt6mu3qh4de2id.onion/'
...
def search_urls():
    ...
    try:
        response = requests.get(CTI_url)
        content = response.text
        pattern = r"\[LOCKBIT 3\.0 \(New Dark mirror[^\]]*\]\((http[^\s)]+?\.onion)\).*?online"
        urls = list(set(re.findall(pattern, content, re.IGNORECASE)))  # 중복 제거
    except Exception as e:
        print(f"⚠️ 유효한 링크 검색 중 에러 발생: {e}")
        return

    while urls:
        url = urls.pop(0)
        print(f"{url} 로 접속 시도함.")
        try:
            response = requests.get(url, proxies=PROXIES, timeout=30)
            return url
        except (requests.exceptions.ConnectionError, requests.exceptions.Timeout):
            print("⚠️ OFFLINE. 재시도 중...")
        except Exception as e:
            print(f"⚠️ 유효한 링크 검색 중 에러 발생: {e}")
    return
    ...
    try:
        ...
        while not stop_scanning and current_page <= MAX_PAGES:
            ...
            try:
                ...
                print(f"[!] {current_page}페이지 응답 없음. 스캔을 중단합니다.")
                print("새로운 주소 찾기 시도 중...")

                if SEARCH_COUNT > 0: # 횟수가 남아있다면 (1회 이상 시도 가능하게)
                    print("🔄 새로운 주소를 검색하여 재시도합니다...")
                    SEARCH_COUNT -= 1 # 횟수 차감
                    new_url = search_urls()
                    
                    if new_url:
                        TARGET_URL = new_url # 새로운 주소로 교체
                        print(f"✅ 새 주소 발견: {TARGET_URL}")
                        # 현재 루프를 건너뛰고 새 주소로 다시 1페이지부터 시작하려면:
                        current_page = 1 
                        continue 
                    else:
                        print("❌ 유효한 새 링크를 찾지 못했습니다.")
                        break
                else:
                    print("❌ 더 이상 재시도할 수 없습니다. (재시도 횟수 소진)")
                    break
                current_page += 1
